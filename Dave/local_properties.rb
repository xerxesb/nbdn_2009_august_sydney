 ...
 def initialize
  @settings = {
    ...
    #app_config_template = "app.config.vista.template" ;
  	...
  	:osql_connectionstring => "-E -S .\\SQLExpress",
...
@settings[:sql_tools_path] = File.join(ENV['SystemDrive'],'program files','microsoft sql server','90','tools','binn')
@settings[:osql_exe] = File.join("#{@settings[:sql_tools_path]}",'osql.exe')
@settings[:config_connectionstring] = "data source=.\SQLExpress;Integrated Security=SSPI;Initial Catalog=#{@settings[:initial_catalog]}"
...
@settings[:asp_net_account] = "#{ENV["computername"]}\\guest";
